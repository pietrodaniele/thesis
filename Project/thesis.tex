...
\begin{document}
	...
	\iffalse
		...
		Therefore, searching for additional Higgs boson from $pp$ collisions in the two photons final states provides a great opportunity to test BSM physics.
		\fi
 			...
 			\begin{table}[tbp]
 				\centering
					\begin{tabular}{llcc}
						...
						$p_T^{\gamma_1}/m_{\gamma\gamma}^{Truth}$	& Scalar relative $p_T$ cut & > 0.3					& > 0.3    			\\
						$p_T^{\gamma_2}/m_{\gamma\gamma}^{Truth}$	& Scalar relative $p_T$ cut & > 0.25				& > 0.25			\\
						$E_{T}^{cone20\ \gamma_1}/p_T^{\gamma_1}$ 	& Isolation cut 			& $\neg$				& < 0.065 			\\
						$E_{T}^{cone20\ \gamma_2}/p_T^{\gamma_2}$ 	& Isolation cut 			& $\neg$				& < 0.065			\\
						\bottomrule[1.5pt]
					\end{tabular}
